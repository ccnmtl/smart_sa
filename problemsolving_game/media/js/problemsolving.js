...
(function () {
  ...
  ProblemSolveGame.prototype.onLoad = function () {
    ...
    //which ones picked
    for (a in self.game_state.my_issues) {
      ...
      if (issue) {
        ...
        if (mode == 'choose_one') {
          ...
          (filled_out_form) ? addElementClass(issue, 'completed') : removeElementClass(issue, 'completed');
    ...
    if (mode == 'problemsolve_one') {
      ...
      if (hasAttr(self.game_state, 'chosen-issue')) {
        ...
        for (field in problemsolve_state) {
    ...
    if (hasAttr(self.game_state, 'chosen-issue')) {
      var issue = self.getIssueByText(self.game_state['chosen-issue']);
      self.chooseOneIssue(null, issue);
    }
  }
