...
#downloaded from http://www.josh-davis.org/pythonAES
#to work in concert with http://www.josh-davis.org/ecmascrypt
#map from the javascript version
def toNumbers(hexstr):
	return [int(hexstr[c:c+2],16) for c in range(0,len(hexstr),2)]

def fromNumbers(hexarr):
	return ''.join([hex(i)[2:].zfill(2) for i in hexarr])
