...
function adjust_for_print() {
  ...
  map(function (a) { }, getElementsByTagAndClassName('div', 'page_3_pill_info'));
...
function grid(board, n) {
    var w = getElementDimensions(board).w; // width and height of the board
    var h = getElementDimensions(board).h;
    var entiregrid = [];
...
function draw_page() {
    ...
    var positions = grid($('pillbox'), patient_meds.length);
    kill_pills();
    pills = {};
