  ...
  LocalFirefoxSession.prototype.destroyEverything = function () {
    ...
    if (confirm('This will delete all data relating to clients and your own admin account.  Are you sure?')) {
      ...
      for (var a2 in self.permStor.keyDict()) {
        if (!(new RegExp('^' + self.nsBACKUP).test(a2))) {
 ...
 */
  function sha1Hash(msg) {
    ...
    for (var i2 = 0; i2 < N; i2++) {
      ...
      // 3 - main loop
      for (var t1 = 0; t1 < 80; t1++) {
        var s = Math.floor(t1 / 20); // seq for blocks of 'f' functions and 'K' constants
        var T = (ROTL(a, 5) + f(s, b, c, d) + e + K[s] + W[t1]) & 0xffffffff;
        e = d;
        d = c;
        c = ROTL(b, 30);
        b = a;
